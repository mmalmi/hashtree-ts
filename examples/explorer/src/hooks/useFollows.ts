...
async function fetchFollows(pubkey: string): Promise<void> {
  ...
  try {
    ...
    console.error('[useFollows] fetch error', e);
  } finally {
    pendingFetches.delete(pubkey);
  }
}

export function useFollows(pubkey?: string): Follows | undefined {
  ...
  const [follows, setFollows] = useState<Follows | undefined>(() =>
    pubkeyHex ? followsCache.get(pubkeyHex) : undefined
  );

  const handleUpdate = useCallback((f: Follows) => {
    setFollows(f);
  }, []);
  ...
  useEffect(() => {
    ...
    // Always subscribe to receive updates
    const unsub = subscribe(pubkeyHex, handleUpdate);
...
export async function followPubkey(targetPubkey: string): Promise<boolean> {
  const pk = useNostrStore.getState().pubkey;
...
async function publishFollowList(pk: string, follows: string[]): Promise<boolean> {
  try {
    ...
    console.error('[useFollows] publish error', e);
    return false;
  }
}
