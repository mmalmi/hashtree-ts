...
test.describe('Add Folder Navigation', () => {
  ...
  test('debug: check path resolution after clicking folder', async ({ page }) => {
    ...
    // Check if folder appears in UI without reload
    const folderVisible = await page.locator('text=debug-nav-folder').isVisible();
    ...
    await page.locator('text=debug-nav-folder').click();
  ...
  test('should navigate into folder created via Add Folder (directory upload)', async ({ page }) => {
    ...
    // Check if the folder appears in the UI
    const folderLocator = page.locator('a:has-text("upload-test-folder")');
    const folderVisible = await folderLocator.isVisible();
