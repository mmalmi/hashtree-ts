/**
 * Cross-language interoperability test: hashtree-ts <-> nosta
 *
 * This test runs a Nosta instance in background and verifies that
...
test.describe('Nosta Interoperability', () => {
  test.setTimeout(90000); // 90 second timeout

  test('hashtree-ts and nosta can exchange messages via temp.iris.to', async () => {
    ...
    for (let i = 0; i < 15; i++) {
      ...
      await pool.publish([TEST_RELAY], newHelloEvent);
    }
    ...
    // Even if no nosta peers are currently online, the protocol works
