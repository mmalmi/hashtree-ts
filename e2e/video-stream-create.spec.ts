...
test.describe('Video Stream Create Page', () => {
  ...
  test('viewer watches full 20+ second stream with live updates', async ({ browser }) => {
    ...
    if (await streamerFollowBtn.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      await expect(streamer.getByRole('button', { name: 'Following' })).toBeVisible({ timeout: 10000 });
    }
    ...
    if (await viewerFollowBtn.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      await expect(viewer.getByRole('button', { name: 'Following' })).toBeVisible({ timeout: 10000 });
    }
    ...
    for (let i = 0; i < 60 && !videoReady; i++) {
      ...
      if (state.readyState >= 1 && state.duration > 0) {
        videoReady = true;
      }
    }
