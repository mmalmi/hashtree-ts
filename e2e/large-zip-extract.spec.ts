...
test.describe('Large ZIP extraction', () => {
  test.skip(!!process.env.CI, 'Skipped in CI - takes too long');

  test('should extract large ZIP file (200MB, 1200+ files) with progress', async ({ page }) => {
    ...
    // Load the large ZIP file - use path directly (Playwright buffer limit is 50MB)
    const zipPath = path.join(process.cwd(), 'test-data', 'WebGLSamples.github.io-master.zip');
    ...
    // Should show file count (1236 files)
