...
export class NDK extends EventEmitter<{
   ...
   * ```
   */
  set signatureVerificationFunction(
    fn: ((event: NDKEvent) => Promise<boolean>) | undefined
  ) {
  ...
  get signatureVerificationFunction():
    | ((event: NDKEvent) => Promise<boolean>)
    | undefined {
  ...
  private getTransportsForSubscription(
    ...
    // In P2P-only mode: signaling → relays + peers, everything else → peers only
    ...
    if (p2pOnlyMode) {
      ...
      // Signaling always uses relays for discovery
      if (isSignaling) {
        transports.push("relays")
      }

      // Add transport plugins
  ...
  public subscribe(
    ...
    this.aiGuardrails?.subscription?.created(
