...
export default class NDKCacheAdapterDexie implements NDKCacheAdapter {
    ...
    const filterFn =
      typeof filter === "function"
        ...
        : (pubkey: Hexpubkey, profile: NDKUserProfile) => {
            ...
            const fields =
              filter.fields ||
              (filter.field ? [filter.field] : ["name", "displayName", "nip05"])
  ...
  public async saveUsersLNURLDoc(
    pubkey: Hexpubkey,
    doc: NDKLnUrlData | null
  ): Promise<void> {
    try {
   ...
   */
  public async getModuleCollection<T>(
    namespace: string,
    collection: string
  ...
  public async getDecryptedEvent(wrapperId: NDKEventId): Promise<NDKEvent | null> {
    try {
      ...
      console.error(
        `[cache-dexie] Error getting decrypted event for wrapper ${wrapperId}:`,
        e
      )
      return null
    }
   ...
   */
  public async addDecryptedEvent(
    ...
    try {
      ...
      console.error(
        `[cache-dexie] Error adding decrypted event for wrapper ${wrapperId}:`,
        e
      )
    }
  }
...
export function foundEvent(
  ...
  try {
    ...
    const relay = relayUrl && subscription.pool ? subscription.pool.getRelay(relayUrl, false) : undefined
    ndkEvent.relay = relay
    if (subscription.eventReceived) {
