...
describe('HashTree', () => {
  ...
  describe('create', () => {
    it('should store small file as single blob', async () => {
      ...
      const { cid, size } = await tree.putFile(data, { public: true });
    ...
    it('should chunk large files', async () => {
      ...
      const { cid, size } = await smallTree.putFile(data, { public: true });
  ...
  describe('read', () => {
    ...
    it('should list directory', async () => {
      const { cid: fileCid } = await tree.putFile(new TextEncoder().encode('data'), { public: true });
      const { cid: dirCid } = await tree.putDirectory([{ name: 'file.txt', cid: fileCid, size: 4 }], { public: true });
    ...
    it('should resolve path', async () => {
      ...
      const { cid: rootCid } = await tree.putDirectory([{ name: 'subdir', cid: subDirCid, size: 6, isTree: true }], { public: true });
  ...
  describe('edit', () => {
    it('should add entry to directory', async () => {
      ...
      const { cid: fileCid, size } = await tree.putFile(new TextEncoder().encode('hello'), { public: true });
    ...
    it('should update existing entry', async () => {
      const { cid: file1 } = await tree.putFile(new TextEncoder().encode('v1'), { public: true });
      const { cid: rootCid } = await tree.putDirectory([{ name: 'file.txt', cid: file1, size: 2 }], { public: true });

      const { cid: file2, size } = await tree.putFile(new TextEncoder().encode('v2 updated'), { public: true });
    ...
    it('should remove entry', async () => {
      const { cid: file1 } = await tree.putFile(new TextEncoder().encode('a'), { public: true });
      const { cid: file2 } = await tree.putFile(new TextEncoder().encode('b'), { public: true });
    ...
    it('should rename entry', async () => {
      const { cid: fileCid } = await tree.putFile(new TextEncoder().encode('content'), { public: true });
      const { cid: rootCid } = await tree.putDirectory([{ name: 'old.txt', cid: fileCid, size: 7 }], { public: true });
    ...
    it('should move entry between directories', async () => {
      ...
      const { cid: rootCid } = await tree.putDirectory([
        { name: 'dir1', cid: dir1Cid, size: 7, isTree: true },
        { name: 'dir2', cid: dir2Cid, size: 0, isTree: true },
      ], { public: true });
    ...
    it('should handle nested path edits', async () => {
      ...
      const { cid: aCid } = await tree.putDirectory([{ name: 'b', cid: bCid, size: 0, isTree: true }], { public: true });
      const { cid: rootCid } = await tree.putDirectory([{ name: 'a', cid: aCid, size: 0, isTree: true }], { public: true });
  ...
  describe('encrypted (default)', () => {
    ...
    it('should preserve isTree flag in encrypted directory', async () => {
      ...
      const { cid: rootCid } = await tree.putDirectory([
        { name: 'subdir', cid: subDirCid, size: 0, isTree: true },
      ]);
    ...
    it('should preserve isTree=false for files in encrypted directory', async () => {
      const { cid: fileCid } = await tree.putFile(new TextEncoder().encode('data'));
      const { cid: dirCid } = await tree.putDirectory([
        { name: 'file.txt', cid: fileCid, size: 4, isTree: false },
      ]);
    ...
    it('should handle nested encrypted directories', async () => {
      ...
      const { cid: bCid } = await tree.putDirectory([
        { name: 'file.txt', cid: fileCid, size: 6, isTree: false },
      ]);

      const { cid: aCid } = await tree.putDirectory([
        { name: 'b', cid: bCid, size: 6, isTree: true },
      ]);

      const { cid: rootCid } = await tree.putDirectory([
        { name: 'a', cid: aCid, size: 6, isTree: true },
      ]);
    ...
    it('should resolve path through encrypted tree with resolvePath', async () => {
      ...
      const { cid: bCid } = await tree.putDirectory([
        { name: 'file.txt', cid: fileCid, size: 8, isTree: false },
      ]);

      const { cid: aCid } = await tree.putDirectory([
        { name: 'b', cid: bCid, size: 8, isTree: true },
      ]);

      const { cid: rootCid } = await tree.putDirectory([
        { name: 'a', cid: aCid, size: 8, isTree: true },
      ]);

      // Resolve the file path
      const resolved = await tree.resolvePath(rootCid, ['a', 'b', 'file.txt']);
      expect(resolved).not.toBeNull();
      expect(resolved!.isTree).toBe(false);
