<script lang="ts">
  ...
  let { text, maxLines = 4, maxChars = 400, class: className = '' }: Props = $props();
...
<div class={className}>
  <p class="whitespace-pre-wrap break-words">{#each segments as segment, i (i)}{#if segment.type === 'link'}<a href={segment.url} target="_blank" rel="noopener noreferrer" class="text-accent hover:underline">{segment.url}</a>{:else if segment.type === 'timestamp'}<button
